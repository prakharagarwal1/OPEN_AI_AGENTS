  ...
  middleware() {
    return async (req: Request, res: Response, next: NextFunction) => {
      ...
      try {
        ...
        const remainingPoints = (rejRes as any).remainingPoints;
        res.set("Retry-After", String(Math.ceil((rejRes as any).msBeforeNext / 1000) || 60));
        res.set("X-RateLimit-Limit", String(config.RATE_LIMIT_MAX_REQUESTS));
        res.set("X-RateLimit-Remaining", String(remainingPoints ?? 0));
        res.status(429).json({
